import com.noxcrew.mcc.commons.base.i18n.I18n
import com.noxcrew.mcc.commons.base.inject

mccScript {
    val i18n: I18n by inject()

    // NPC roaming near HITW back slime walls.
    playerNPC(
        absoluteVec(-777.0, 34.0, 1254.5),
        "Assembler Stanley",
        "",
        ""
    ) {
        rotation(-90.0, 0.0)
        val pointList = LinkedList<Transform>()
        pointList += Transform(-775.0, 34.0, 1254.5)
        pointList += Transform(-780.0, 34.0, 1254.5)
        pointList += Transform(-779.0, 34.0, 1245.0)
        pointList += Transform(-764.0, 34.0, 1242.5)
        pointList += Transform(-760.0, 34.0, 1237.5)
        pointList += Transform(-737.0, 34.0, 1237.5)
        pointList += Transform(-734.5, 34.0, 1241.0)
        pointList += Transform(-734.5, 34.0, 1259.5)
        patrol(pointList)
        type("ambient")
        offHand("mcc:island_cosmetics.accessory.pipe_wrench{display:{color:15079705}}")
    }
}
