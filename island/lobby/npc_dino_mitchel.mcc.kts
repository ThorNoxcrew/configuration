import com.noxcrew.mcc.commons.base.i18n.I18n
import com.noxcrew.mcc.commons.base.inject

mccScript {
    val i18n: I18n by inject()

// Dinosaur hat-wearing anthropologist by the fossils. Looks after the kids playing around in dinosaur onesies.
    playerNPC(
        absoluteVec(189.0, 42.0, 426.0),
        "Mitchel Brown",
        "",
        ""
    ) {
        rotation(120.0, 0.0)
        lookClosePerPlayer(6)
        cooldown(1.seconds)

        type("ambient")
        helmet("mcc:island_cosmetics.hat.dino{display:{color:6566410}}")
    }
}
