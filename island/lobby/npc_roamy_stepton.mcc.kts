import com.noxcrew.mcc.commons.base.i18n.I18n
import com.noxcrew.mcc.commons.base.inject
import java.util.LinkedList

mccScript {
  val i18n: I18n by inject()

  playerNPC(
      absoluteVec(358.5, 36.0, 536.5),
      "<glyph=mcc:icon.emojis.sweat_smile> Roamy Stepton",
      "",
      "") {
    rotation(75.0, 0.0)

    val pointList = LinkedList<Transform>()
    pointList += Transform(347.5, 35.5, 519.5)
    pointList += Transform(332.5, 37.0, 518.5)
    pointList += Transform(335.5, 36.0, 507.5)
    pointList += Transform(331.0, 35.5, 506.5)
    pointList += Transform(332.0, 35.0, 498.5)
    pointList += Transform(351.5, 33.5, 496.0)
    pointList += Transform(342.0, 33.0, 476.5)
    pointList += Transform(340.5, 34.0, 460.5)
    pointList += Transform(313.5, 35.0, 463.5)
    pointList += Transform(322.5, 35.0, 503.5)
    pointList += Transform(288.5, 37.5, 499.5)
    pointList += Transform(283.5, 34.5, 449.5)
    pointList += Transform(300.5, 34.5, 442.5)
    pointList += Transform(324.5, 33.5, 439.5)
    pointList += Transform(324.5, 31.5, 406.5)
    pointList += Transform(351.5, 31.5, 408.5)

    patrol(pointList)

    type("ambient")
  }
}
