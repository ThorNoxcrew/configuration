import com.noxcrew.mcc.commons.base.i18n.I18n
import com.noxcrew.mcc.commons.base.inject

mccScript {
    val i18n: I18n by inject()

// Alex's skin roaming on a park pedistol in back right of HUB. Alex no longer works for Noxcrew, and even still we should refrain from using things that are too referencial.
    playerNPC(
        absoluteVec(255.0, 39.0, 541.5),
        "City Monster-Man",
        "",
        ""
    ) {
        roaming(absoluteVec(256.5, 39.0, 543.5), absoluteVec(252.5, 39.0, 539.5))

        type("ambient")
    }
}
